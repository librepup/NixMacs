...
#+END_SRC
** ANSI Colors
Correctly set ANSI Colors for Interpretation of ANSI Escape Codes, and force-refresh according to current Theme.
#+BEGIN_SRC emacs-lisp
(require 'ansi-color)
...
** Autosaves
Send autosaved files to a specific directory so they don't pollute every working directory you ever exit
a file in without saving.
#+BEGIN_SRC emacs-lisp
...
(defun my/maybe-show-splash ()
  "Show splash screen only if no files were opened from the command line."
  (unless (> (length (window-list-1 nil nil t)) 1)
    (my-custom-splash-screen)))

(add-hook 'emacs-startup-hook #'my/maybe-show-splash)
